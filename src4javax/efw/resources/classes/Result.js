 ...
 * @author Chang Kejun
 */
class Result extends Debuggable{
	/**
	 * Create instance
	 * 
	 * @param {boolean}
	 *            withoutI18nTranslation: optional
	 */
	constructor(withoutI18nTranslation) {
		super();
	...
	remove(selector) {
		if (this.values.length > 0) {
			let ret = this.values[this.values.length - 1];
	 ...
	 * @returns {Result}
	 */
	confirm(message, buttons, title, params) {
		if (typeof(title)=="object"){
			params=title;
			title=null;
		}
