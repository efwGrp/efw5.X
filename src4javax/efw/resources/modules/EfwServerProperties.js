"use strict";
/**** efw5.X Copyright 2025 efwGrp ****/
/**
 * The class to operate efw.properties file.
 * 
 * @author Chang Kejun
 */
class EfwServerProperties extends Debuggable{
	/**
	 * The function to get String value from efw.properties file.
	 * 
	 * @param {String}
	 *            key: required<br>
	 * @param {String}
	 *            defaultValue: optional<br>
	 * @returns {any}
	 */
	static get(key, defaultValue) {
		let dv;
		if (defaultValue == null) {
			dv = null;
		} else {
			dv = defaultValue + "";
		}
		let value=""+Packages.efw.properties.PropertiesManager.getProperty(key, dv);
		if(defaultValue==null){
			return value;
		}else if(typeof defaultValue=="string"){
			return value;
		}else if(typeof defaultValue=="number"){
			return 0+new Number(value);
		}else if(typeof defaultValue=="boolean"){
			return true && java.lang.Boolean.parseBoolean(value);
		}else{
			return value;
		}
	}
}
///////////////////////////////////////////////////////////////////////////////
const properties = EfwServerProperties;