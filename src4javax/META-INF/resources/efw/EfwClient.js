...
class EfwClient{
	...
	async fire(eventParams) {
		...
		var servletUrl = efw.baseurl + "/efwServlet";
		var uploadUrl = efw.baseurl + "/uploadServlet";
		var downloadUrl = efw.baseurl + "/downloadServlet";
		var previewUrl = efw.baseurl + "/previewServlet";
		try {
			//first calling
			this.#consoleLog("First calling parameters", eventParams);
			var paramsFormat = await this.#callFirstAjax(servletUrl, eventId);
			this.#consoleLog("First calling result", paramsFormat);
			// auto collect params
			// the first calling return data is paramsformat,use it to get params
			var params = this.#pickupParams(eventId, paramsFormat, null, manualParams);
			// upload files
			if (this.#pickupParams_uploadformdata != null) {
				await this.#callUploadAjax(uploadUrl);
				this.#pickupParams_uploadformdata = null;// reset it for next ajax.
			}
			// second calling
			this.#consoleLog("Second calling parameters", params);
			var result = await this.#callSecondAjax(servletUrl, eventId, params);
			this.#consoleLog("Second calling result", result, true);
			//auto show values
			...
			if (e.errorTitle) {
				this.#consoleLog(e.errorTitle, e.errorData);
			}
		}
	}
	 ...
	 */
	#callFirstAjax(servletUrl, eventId) {
		var self = this;
	...
	#callUploadAjax(uploadUrl) {
		...
		return new Promise(function(resolve, reject) {
			$.ajax(self.#options = {
				...
				processData: false,
				contentType: false,
				data: self.#pickupParams_uploadformdata,// upload
				// files
	 ...
	 */
	#callSecondAjax(servletUrl, eventId, params) {
		var self = this;
					...
					if (result.actions.error != null && result.actions.error.clientMessageId == "EventIsBusyException") {
						...
						if (service != null) {
							...
							if (service.retriable) {
								...
								efw.dialog.wait(message, countdown, null, function() { $.ajax(self.#options); });
							} else {
	 ...
	 */
	#pickupParams_uploadformdata = null;
	/**
	 ...
	 * @returns {Object}
	 */
	#pickupParams(eventId, paramsFormat, context,
		manualParams) {
		try {
			var data = {};
			for (var key in paramsFormat) {
				...
				if (format == null || $.type(format) == "string") {
					...
					vl = this.#pickupParams(eventId, format, element,
						manualParams);
				} else {
	 ...
	 * @param data
	 */
	#consoleLog(msg, data, showThreadLogs) {
		if (window.console) {
			var date = new Date();
