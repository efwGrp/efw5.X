...
Function.prototype.debug = function(label) {
	if (!label)
		label = "";
	java.lang.System.out.println("-----" + label + "-----");
	java.lang.System.out.println("This is "+this.name+".");
	return this;
};
...
Number.prototype.format=function(formatter, rounder){
	if (formatter.indexOf("{")==0&&formatter.lastIndexOf("}")==formatter.length-1){
		return EfwServerFormat.formatEnum(this,formatter);
	}else{
		return EfwServerFormat.formatNumber(this,formatter, rounder);
	}
};
...
Number.parse=function(value,formatter){
	if(formatter==null){
		...
		return EfwServerFormat.parseEnum(value, formatter);
	}else{
		return EfwServerFormat.parseNumber(value, formatter); 
	}
};
...
Date.prototype.format=function(formatter){
	if (formatter.indexOf("{")==0&&formatter.lastIndexOf("}")==formatter.length-1){
		...
		return EfwServerFormat.formatDate(this,formatter);
	}
};
...
Date.parse=function(value,formatter){
	if(formatter==null){
		...
		return EfwServerFormat.parseDate(value, formatter); 
	}
};
...
Date.prototype.getYears=function(current){
	...
	let diff=Number.parse(current.format("yyyyMMdd"),"0")-Number.parse(this.format("yyyyMMdd"),"0");
...
///////////////////////////////////////////////////////////////////////////////
/**
 * The parent class for all to add a common debug function 
 */
class Debuggable{
	debug(label){
		if (!label)
			label = "";
		java.lang.System.out.println("-----" + label + "-----");
		java.lang.System.out.println("This is an instance of "+this.constructor.name+".");
		return this;
	}
	static debug(label){
		if (!label)
		...
		java.lang.System.out.println("This is "+this.name+ " class.");
		return this;
	}
}