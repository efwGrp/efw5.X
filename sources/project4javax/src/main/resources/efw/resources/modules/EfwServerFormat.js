 ...
 * @author Chang Kejun
 */
class EfwServerFormat extends Debuggable{
	/**
	...
	static formatNumber (value, formatter, rounder) {
		  ...
		  let ret= ""+Packages.efw.format.FormatManager.formatNumber(value, formatter,
																  rounder);
		  if(isFullWidth){
						ret=ret
						.replace(/0/g,"０")
	...
	static formatDate (value, formatter) {
		...
		let ret = "" + Packages.efw.format.FormatManager.formatDate(value.getTime(),
						formatter);
		if(isFullWidth){
				ret=ret
				.replace(/0/g,"０")
	 ...
	 * @returns {String}
	 */
	static formatEnum (value, formatter) {
		let _value=""+value;
		...
		let ary=formatter.split(",");
		for(let i=0;i<ary.length;i++){
			let item=ary[i].split("=");
	 ...
	 */
	static parseEnum (value, formatter) {
		let _value=""+value;
		if (formatter==null) return _value;//formatter未設定の場合文字列に変換するだけ。
		if (formatter.indexOf("{")!=0) return _value;
		if (formatter.lastIndexOf("}")!=formatter.length-1) return _value;
