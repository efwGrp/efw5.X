 ...
 * @author Chang Kejun
 */
class EfwServerMessages extends Debuggable{
	static get(messageId,lang){
	...
	static translate(jsonString,lang) {
		if(lang==null){
			return jsonString;
		}else{
			let keys=jsonString.match(/\{[\w]+\}/g);
			if (keys!=null){
				for(let i=0;i<keys.length;i++){
					let key=keys[i];
					key=key.substr(1,key.length-2);
					let msg=Packages.efw.i18n.I18nManager.get(lang,key,"");
					if(msg==""){
...
///////////////////////////////////////////////////////////////////////////////
const messages = EfwServerMessages;