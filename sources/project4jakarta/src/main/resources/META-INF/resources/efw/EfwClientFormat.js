...
class EfwClientFormat {
	 ...
	 * @returns {String}
	 */
	static formatNumber(value, formatter, rounder) {
		var ft = EfwClientFormat.#initNumberFormat(formatter);// get formatter object
		...
		var numberStr = ""
				+ EfwClientFormat.#roundNumber(num, ft.maxFrac, rounder);
		if (isNaN(numberStr))
			return "";// if it is not number return ""
	 ...
	 * @returns {null | Number}
	 */
	static parseNumber(value, formatter) {
		...
		if (pIndex == 0 && sIndex > 0) {
			...
			if (isPercentage) {
				value = value / 100;
				value = EfwClientFormat.#roundNumber(value,
						ft.maxFrac + 2);
			}
			return value;
		} else {
	 ...
	 * @returns {null | Number}
	 */
	static #roundNumber(num, precision, mode) {
	 ...
	 * @returns {Object}
	 */
	static #initNumberFormat(formatter) {
		var ret = {
	 ...
	 * @returns {String}
	 */
	static formatDate(value, formatter) {
		if (value == null)
			return "";// it value is not null, return ""
		if (!value.getTime)
			return "";// if value is not date, return ""
		var ft = EfwClientFormat.#initDateFormat(formatter);// get formatter object
	
		var ret = [];
		for (var i = 0; i < ft.length; i++) {
			var pattern = ft[i];
			var formatter = EfwClientFormat.#dateFormatter[pattern.charAt(0)];
			if (formatter) {
				ret[i] = formatter.apply(EfwClientFormat, [ value, pattern ]);
			} else {
				ret[i] = pattern;
			}
		}
	 ...
	 * @returns {null | Date}
	 */
	static parseDate(value, formatter) {
		...
		for (var i = 0; i < ft.length; i++) {
			...
			var pattern = ft[i];
			var parser = EfwClientFormat.#dateParser[pattern.charAt(0)];
			if (parser) {
				value = parser.apply(EfwClientFormat, [ value, pattern, ret ]);
			} else {
	 ...
	 * @returns {String}
	 */
	static formatDateToNumber(value, formatter) {
		if (value == null)
			return "";// it value is not null, return ""
		if (!value.getTime)
			return "";// if value is not date, return ""
		...
		for (var i = 0; i < ft.length; i++) {
			...
			var formatter = null;
			if (pattern.charAt(0) != "'") {
				formatter = EfwClientFormat.#dateFormatter[pattern.charAt(0)];
			}
			if (formatter) {
				ret[i] = formatter.apply(EfwClientFormat, [ value, pattern ]);
			} else {
	 ...
	 * @returns {Date}
	 */
	static parseDateFromNumber(value, formatter) {
		value += ""; // change value to string if it is not string
		if (!value)
			return null; // if value is blank return null
		var ft = EfwClientFormat.#initDateFormat(formatter);// get formatter object
		...
		for (var i = 0; i < ft.length; i++) {
			...
			if (pattern.charAt(0) == "y" || pattern.charAt(0) == "M"
					...
					|| pattern.charAt(0) == "S") {
				parser = EfwClientFormat.#dateParser[pattern.charAt(0)];
			}
			if (parser) {
				value = parser.apply(EfwClientFormat, [ value, pattern, ret ]);
				if (value === null)
	...
	static #dateFormatter = {
		"y" : function(date, pattern) {// Year
			...
			if (pattern.length <= 2) {
				...
				year = EfwClientFormat.#zeroPadding(year, pattern.length);
			}
			return year;
		},
		"M" : function(date, pattern) {// Month in year
			return EfwClientFormat.#zeroPadding(String(date.getMonth() + 1), pattern.length);
		},
		"d" : function(date, pattern) {// Day in month
			return EfwClientFormat.#zeroPadding(String(date.getDate()), pattern.length);
		},
		"H" : function(date, pattern) {// Hour in day (0-23)
			return EfwClientFormat.#zeroPadding(String(date.getHours()), pattern.length);
		},
		"m" : function(date, pattern) {// Minute in hour
			return EfwClientFormat.#zeroPadding(String(date.getMinutes()), pattern.length);
		},
		"s" : function(date, pattern) {// Second in minute
			return EfwClientFormat.#zeroPadding(String(date.getSeconds()), pattern.length);
		},
		"S" : function(date, pattern) {// Millisecond
			return EfwClientFormat.#zeroPadding(String(date.getMilliseconds()), pattern.length);
		},
	 ...
	 * @returns {Boolean}
	 */
	static #isPureNumber = function(str) {
		return /^[0-9]*$/.test(str);
	}
	/**
	...
	static #dateParser = {
		"y" : function(text, pattern, result) {// Year
			...
			if (!EfwClientFormat.#isPureNumber(year))
				return null; // error
		...
		"M" : function(text, pattern, result) {// Month in year
			...
			if (!EfwClientFormat.#isPureNumber(month))
				return null; // error
		...
		"d" : function(text, pattern, result) {// Day in month
			...
			if (!EfwClientFormat.#isPureNumber(day))
				return null; // error
		...
		"H" : function(text, pattern, result) {// Hour in day (0-23)
			...
			if (!EfwClientFormat.#isPureNumber(hour))
				return null; // error
		...
		"m" : function(text, pattern, result) {// Minute in hour
			...
			if (!EfwClientFormat.#isPureNumber(min))
				return null; // error
		...
		"s" : function(text, pattern, result) {// Second in minute
			...
			if (!EfwClientFormat.#isPureNumber(sec))
				return null; // error
		...
		"S" : function(text, pattern, result) {// Millimsecond
			...
			if (!EfwClientFormat.#isPureNumber(msec))
				return null; // error
	 ...
	 * @returns {Array}
	 */
	static #initDateFormat(formatter) {
		var ret = [];
...
Number.prototype.format=function (formatter, rounder){
	return EfwClientFormat.formatNumber(this,formatter, rounder);
};
/**
...
Number.parse=function(value,formatter){
	if(formatter==null){
		...
		return EfwClientFormat.parseNumber(value, formatter); 
	}
};
/**
...
Date.prototype.format=function(formatter){
	return EfwClientFormat.formatDate(this,formatter);
};
/**
...
Date.parse=function(value,formatter){
	if(formatter==null){
		...
		return EfwClientFormat.parseDate(value, formatter); 
	}
};
/**
